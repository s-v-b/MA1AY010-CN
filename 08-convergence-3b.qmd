# Refinments and extensions of thr Central Limit Theorem {#chaprevisiCLT}
...
In Section \@ref(secMetrisationWeakConv), we associate convergence in distribution
with metrics. By metrizing convergence in distribution, we can quantify rates of
convergence.

In Section \@ref(steinsmethod) we turn a relatively recent approach to weak convergence: Stein's method.
...
The next  theorem is a corollary  of Theorem \@ref(thm:thmLevyInj).
...
## Metrizations of weak convergence {#secMetrisationWeakConv}
...
```{definition defKolLev, name="Kolmogorov-Lévy distance"}
...
\begin{displaymath}
  \inf \Big\{\epsilon : \epsilon >0,   \qquad  P(-\infty, x-\epsilon] -\epsilon \leq Q(-\infty, x] \leq P(-\infty, x-\epsilon] + \epsilon
  \Big\} \, .
\end{displaymath}
...
```{definition, name="Wasserstein distance"}
...
pour $\epsilon>0$, on définit $h_{x, \epsilon}$ par \begin{equation*}
  h_{x,\epsilon}(w) =
  \begin{cases}
  1 & \text{si } x\geq w\\
...
```{proposition, name="Identité de Stein"}
...
```{proof}
See  Lemmae \@ref(lem:stein) and \@ref(lem:stein:bis).
```
...
a  solution of the differential equation stated in Lemma \@ref(lem:ode1)
...
## A Berry-Esseen bound for a Lindeberg-Feller-like Central Limit Theorem  {#secBerryLindeberg}



Le theorem suivant peut etre lu comme un résultat intermédiaire entre
le theorem de Berry-Esseen \@ref{thm:tcl:berry} (il donne une distance à la loi normale)
et le theorem central limite de Lindeberg-Feller \@ref{thm:tcl:lindeberg}
...
```{proof, name="Proof of Theorem \@ref(thm:lindeberg:feller:stein)"}
...
(ref:quantcltbinom2) Kolmogorov distance between standard Gaussian and distribution of $(X_n - np)/\sqrt{np(1-p}$ with $X_n$  binomially distributed with parameters $n$ and $p=.3$. The rate of convergence in the central limit theorem is asymptotically of order $1/\sqrt{n}$.


```{r quantcltbinom2, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='(ref:quantcltbinom2)'}
...
tibble(n=unlist(l_n), y=dks, z=sqrt(l_n[[1]])/sqrt(unlist(l_n))) %>%
  ggplot(mapping=aes(x=n, y=y)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10() +
  geom_line(mapping=aes(y=z*dks[1]), linetyp='dashed') +
  xlab("n") +
  ylab("Kolmogorov distance to standard Gaussian")
...
Section \@ref() follows the first  pages of [@2011arXiv1109.1880R]

La démonstration complète du Lemme \@ref{lem:regu:stein}
