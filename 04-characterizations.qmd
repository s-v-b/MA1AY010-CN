...
Now, by dominated convergence (@thm-thmdominatedConv), 
$$
...
@thm-fouriermoments asserts  that integrability of $X$ is enough to warrant that $\widehat{F}^{(1)}(0)$ is well-defined and that at $t=0$ 
...
and that a similar formula holds for higher order derivatives. 

Like many other results in this area, @thm-fouriermoments is about interchanging derivation and expectation. The fact that the proof relies the dominated convergence theorem (@thm-thmdominatedConv) is not surprising.
...
but for all $x \in \mathbb{R}$, by dominated convergence (@thm-thmdominatedConv),
...
@rem-fourier-derivation is relevant here again. 
...
Check that $\int_0^1 \cos(sx) (1-s) \mathrm{d}s\geq 0$ for all $x \in \mathbb{R}$.
