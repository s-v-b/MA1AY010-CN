# Characterizations of probability distributions  {#chapchar}
...
generating functions (Section \@ref(PGFbis)), Laplace transforms (Section \@ref(laplace)) and characteristic functions
which extend Fourier transforms to probability distributions (Section \@ref(charfun)).
The three methods are distinct in scope but they rely on the same idea and
...
The three methods provide us with a seamless treatment
of sums of independent random variables.

All three methods relate
the integrability of probability distributions and the smoothness of transforms.

In the next lessons (Chapter \@ref(chaprevisiCLT)), we shall see that the three transforms characterize
...
## Probability generating function  {#PGFbis}
...
::: {#def #defpgf name="Probability Generating Function"}
...
::: {.corollary #invpgf name="Inversion formula"}
...
A PGF is infinitely many times inside the (open) disk of convergence. If
...
:::{#thm #pgfintegr name="Integrability and probability generating functions"}
...
::: {#thm #invLaplace name="Widder's Theorem"}
...
::: {#thm #bennettpoisson name="Tail bounds for Poisson distribution"}
...
- See Section \@ref(jensensec) for the notion of convex duality.
...
G_{X+Y}(\lambda)
  ...
  & = \mathbb{E}\Big[\mathrm{e}^{\lambda X} \Big] \times \mathbb{E}\Big[\mathrm{e}^{\lambda Y}\Big] & \text{independence}\\
...
## Characteristic functions and Fourier transforms {#charfun}
...
<br> <p> </p>
Note that Theorem \@ref(thm:invfourier1) does not deliver directly the distribution function $F$. Indeed, if $F$ is not continuous,
...
We have established Theorem \@ref(thm:injeccharfunc).
...
::: {#thm #invformula name="Inversion formula"}
...
<br> <p> </p>
The proof of Theorem \@ref(thm:invformula) can be found in textbooks like [@Dur10] or [@MR2893652].
...
::: {#thm #steinIdentity name="Stein's identity"}
...
\]
The proof is completed by invoking Theorem \@ref(thm:fouriermoments).
...
::: {#def #quantfun name="Quantile function"}
...
::: {#prp name="Quantile transformation"}
...
(ref:quantiles) Quantile functions of binomial distributions.
...
(ref:quantiles2) Quantile functions $\max(X, \tau)$ where $X \sim \mathcal{N}(0,1)$ for $\tau \in \{0, 2\}$.  Let $\Phi^\leftarrow$ denote the quantile function of $\mathcal{N}(0,1)$. The quantile  function of $\max(X,\tau)$ is $\mathbb{I}_{(0,\Phi(\tau)]}(p) \times \tau + \Phi^{\leftarrow}(p) \times \mathbb{I}_{(\Phi(\tau), 1)}(p)= \Phi^{\leftarrow}(p \vee \Phi(\tau))$. The two distributions are neither absolutely continuous nor discrete.
...
(ref:quantiles3) Cumulative distribution functions for the probability distributions illustrated in Figure \@ref(fig:quantiles2)
...
(ref:qqplot) Quantile-Quantile plot (QQ-plot). This is a popular device for comparing two probability distributions on the real line defined by their cumulative distributions $F$ and $G$. The QQ-plot consists in plotting function $G^{\leftarrow} \circ F$. If the two distributions are the same, the line $y = G^{\leftarrow} \circ F(x)$ should lie below $y=x$ (according to Proposition \@ref(prp:quantilekit)), and should even coincide with the diagonal if $F=G$ is continuous and increasing. Here $F$ is the cumulative distribution function of $\mathcal{N}(0,1)$. And $G$ is the empirical distribution of a sample of size $500$ of $\mathcal{N}(0,1)$.
...
Prove Proposition \@ref(cor:corscore)

:::


## Bibliographic remarks {#bibcharac}
