...
In table \@ref(tab:toyhashing), line $\omega$ represents the outcome of a random allocation
...
it has to be a $\sigma$-_algebra_, see Section \@ref(tribe).
...
This function is a special case of a _random variable_ see Section \@ref(measurablefunctionsrv).

In the toy example outlined in Table \@ref(tab:toyhashing),
...
see Section \@ref(bernoulli) for more on Bernoulli distributions. This comes from
...
The notion of _independence_ is a cornerstone of probability theory, see Chapter \@ref(independence).
...
In the toy example described in Table \@ref(tab:toyhashing), $Y_3(\omega) = 4$ while $Y_5(\omega)=1$
and $Y_4(\omega)=0$:
...
_Binomial distribution_, see Section \@ref(bernoulli). 
...
See Chapter \@ref(moments) for a systematic approach to expectation, variance and higher moments,
based on Integration theory.

The last chapter (Chapter \@ref(chapConcentration)) is dedicated
...
This is illustrated by Figure \@ref(fig:binom-pmf). In principle,
a binomial random variable with parameters $n=5000$ and $p=.001$
can take any value between $0$ and $5000$. However, most (more than $95\%$) of the probability mass
is supported by  $\{1, \ldots, 10\}$.



```{r  binom-pmf, echo=FALSE, fig.cap="Probability mass function of Binomial(5000,0.001)"}
# %%
n <- 5000
m <- 1000
...
This is an instance of the _law of rare events_, a special case of _convergence in distribution_  see Chapter \@ref(chapweakdistrib).

The ability to approximate a Poisson distribution using
an appropriate Binomial distribution is illustrated in Figure \@ref(fig:binom-poisson). The difference between the
...
```{r  binom-poisson, warning=FALSE, echo=FALSE, fig.cap="Probability mass functions of Binomial(250,0.02) (left), Binomial(2500,0.002) (middle) and Poisson(5) (right)", fig.show='hold'}
# %%
k <- 0:20
...
A general definition is provided in Chapter \@ref(chapweakdistrib).
In Figure \@ref(fig:lawrareevents), this distance between
...
that the variation distance decays like $1/n$. This is checked in Chapter \@ref(chapweakdistrib).


```{r lawrareevents, echo=FALSE, fig.cap="Law of rare events: distance between Binomial(n, 5/n) and Poisson(5) as a function of n"}
...
a lot of information about its moments and distribution. If we let again $n,m$ tend to infinity while $n/m$ tends toward $c>0$, we observe that the distribution of $K_{n,r}/m$ tends to concentrate around $\mathrm{e}^{-c}	\frac{c^r}{r !}$. This is an example of _convergence in probability_, see Chapter \@ref(convergences1).
...
(see Section \@ref(distribfun))  converges pointwise towards the distribution function of the Gaussian distribution.
