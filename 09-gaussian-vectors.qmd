...
_Gaussian vector_ iff for any real vector  $(\lambda_1, \lambda_2, \ldots, \lambda_n)^T$, the distribution of the univariate random variable $\sum_{i = 1}^n \ \lambda_i X_i$ is Gaussian.
...
According to proposition @prp-densite-gamma),
...
$$\left(\sum_{j\in \mathcal{I}} \langle \alpha_j, \pi_{E_j} X \rangle, \sum_{j\in \mathcal{J}} \langle \beta_j, \pi_{E_j} X \rangle\right)$$ can be factorized. It suffices to check that the two Gaussians are orthogonal.
...
According to Cochran's Theorem (@sec-cochran), random vectors  $(\overline{X}_n, \ldots , \overline{X}_n)^t$,
