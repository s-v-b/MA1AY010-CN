# Gaussian vectors  {#chapGaussianVectors}


## Univariate Gaussian distribution {#secGaussUnivar}
...
```{lemma,  steinslemma, name="Stein's Lemma"}
Let $X \sim \mathcal{N}(0,1)$, let $g$ be an absolutely continuous function with derivative $g'$ such that
$\mathbb{E}[ |X g(X)|]<\infty$, then
...
provides us with a converse or Lemma \@ref(lem:steinslemma).


```{lemma, steinslemmabis, name="Stein's Lemma (bis)"}
...
```{proposition,  millsratio, name="Tail probabilities"}
...
```{proposition, gaussmoments, name="Moments"}
...
## Gaussian vectors  {#secGaussVec}


A Gaussian vector is a collection of univariate Gaussian random variables that
satisfies a very stringent property:

```{definition,  gaussVector, name="Gaussian Vector"}
A random vector $(X_1, \ldots, X_n)^T$ is a
_Gaussian vector_ iff for any real vector  $(\lambda_1, \lambda_2, \ldots, \lambda_n)^T$, the distribution of
the univariate random variable $\sum_{i = 1}^n \ \lambda_i X_i$ is Gaussian.
```
...
$(X_1, \ldots, X_n)^t$ is a Gaussian vector.
```
...
\[ K [i, j] = \operatorname{Cov} (X_i, X_j) = \mathbb{E} [X_i X_j] -
...
For any  $\lambda \in \mathbb{R}^p$, $\langle \lambda , A Y \rangle = \langle A^T \lambda, Y \rangle$ , thus
$A \times Y$  is  Gaussian with variance
\[
...
```{definition sdp, name="Semi-definite positive matrices"}
...
```{proposition, choleskyfac, name="Cholesky's factorization"}
...
```{definition, gaussspace, name="Gaussian space"}
...
## Convergence of Gaussian vectors  {#convergenceGaussVectors}


Recall the Lévy continuity theorem (Theorem \@ref(thm:thmlevycont)), it relates weak convergence for probability measures
and simple convergence for characteristic functions.
...
For every $\vec{s} \in \mathbb{R}^k$, functions $\vec{x}
\mapsto \cos ( \left\langle \vec{s}, \vec{x} \right\rangle)$ and
$\vec{x} \mapsto \sin ( \left\langle \vec{s}, \vec{x}
\right\rangle)$ are continuous and bounded, They are also infinitely many times differentiable.
It is remarkable and useful that weak convergence can be checked
on this small set of functions.

```{theorem, name="Lévy-Continuity Theorem, bis"}
...
$\vec{s} \in \mathbb{R}^k$:
\[ \mathbb{E}_{P_n} \left[ \mathrm{e}^{\imath \langle \vec{s},
\vec{X} \rangle} \right] \rightarrow f(\vec{s}) . \]
...
$(X_{k+1}, \ldots, X_n)^T$ with respect to $\sigma(X_{1},\ldots,X_k)$
and whose variance is the Schur complement of the covariance of $(X_{1},\ldots,X_k)^T$
...
W - B A^{-1} B^t
\]
is positive definite. Sub-matrices $A$ and   $W - B A^{-1} B^t$
both have a Cholesky decomposition $A = L_1 L_1^t $,
$W - B A^{-1} B^t = L_2 L_2^t$ where   $L_1, L_2$  are lower triangular,
...
X_k
\end{pmatrix} \, ,
\end{eqnarray*} car $(Y_{k+1}, \ldots,Y_n)^t$ is centered and independent from $\mathcal{G}$.
...
As  $(Y_1,\ldots, Y_k)^t = L_1^{-1}(X_1,\ldots,X_k)^T$,
...
```{definition, name="Gamma distributions"}
...
\geq 0} x^{p - 1} e^{- \lambda x} \] where  $\Gamma (p) \equiv
\int_0^{\infty} t^{p - 1} e^{- t} \mathrm{d} t$.
...
```{proposition, chixdeux, name="Chi-square distributions"}
...
```{proof}
According to proposition \@ref(prop:densite:gamma),
...
```{theorem}
If  ${X}:= (X_1, X_2,
\ldots, X_n)^{^T} \sim \mathcal{N}\left(0, A\right)$ with  $A = L L^T$ ($L$ lower triangular), if
$M \in \mathrm{SDP}(n)$,  then
${X}^T M {X}$ is  distributed like $\sum_{i = 1}^n \lambda_i Z_i$ where
$(\lambda_i)_{i \in \{1, \ldots, n\}}$ denote the eigenvalues of $L^T \times M\times L$
...
```{proof}
Matrix $A$ may be factorized as $A = LL^t$ (Cholesky), and  ${X}$ is  distributed like
$L {Y}$ where ${Y}$ is standard Gaussian. The quadratic form ${X}^T M
{X}$ is thus distributed like ${Y}^T {L}^T M
{L}  {Y}$.  There exist an orthogonal transform
 $O$ such that $L^T M L = O^t \operatorname{diag}
(\lambda_i) O.$ Random vector $O {Y}$ is distributed like
$\mathcal{N} (0, I_n)$.
```

## Norm of non-centred Gaussian vectors
...
```{definition, name="Stochastic ordering"}
...
```{proof}
The  covariance matrix of  $\pi_{E_j} X$  is $\pi_{E_j} \pi_{E_j}^t = \pi_{E_j}$. The eigenvalues of $\pi_{E_j}$  are $1$ with  multiplicity $\text{dim}(E_j)$ and  $0$. The statement about the distribution of $\| \pi_{E_j} X\|_2^2$ is
a corollary of propositions~\@ref(prp:normgaussstand) and~\@ref(prp:normespectre).
...
$$\left(\sum_{j\in \mathcal{I}} \langle \alpha_j, \pi_{E_j} X \rangle, \sum_{j\in \mathcal{J}} \langle \beta_j, \pi_{E_j} X \rangle\right)$$ can be factorized. It suffices to check that the
...
According to Cochran's Theorem (\@ref(cochran)), random vectors  $(\overline{X}_n, \ldots , \overline{X}_n)^t$,
...
```{definition, name="Student's distribution"}
...
\begin{enumerate}
\item if  $f$ is  differentiable on $\mathbb{R}^d$,
\[
\operatorname{var}(f(X)) \leq \mathbb{E} \| \nabla f \|^2   \qquad \text{(Poincaré's inequality)}
\]
\item if  $f$  is  $L$-Lipschitz on  $\mathbb{R}^d$,
\[
...
```{proposition, name="Covariance identity"}
...
The lemmas \@ref(lem:stein) and \@ref(lem:steinbis) that characterize the Gaussian standard are the starting point of Stein's (Charles) method to demonstrate the central limit theorem (and many other results). This relatively recent development is described in [@2011arXiv1109.1880R].
