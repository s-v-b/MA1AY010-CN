# Convergences I : almost sure, $L_2$, $L_1$, in Probability  {#sec-convergences1}
...
we shall borrow and adapt the notions used in Analysis: pointwise convergence ([Section @sec-asconvergence])), convergence in $L_p, 1 \leq p <\infty$ ([Section @sec-Lpconvergence])).

Finally, we  define and investigate _convergence in probability_. This notion weakens both $L_p$ and almost sure (pointwise) convergence. Just as $L_p$ convergences, it can be metrized.

Convergence in probability and almost sure convergence are
illustrated  by weak and strong law of large numbers (Sections @sec-wlln and @sec-slln). Laws of large numbers
...
In [Section @sec-expineq]), we look at non-asymptotic counterparts of the weak law of
...
## Almost sure convergence  {#sec-asconvergence}

The notion of almost sure convergence mirrors the notion of pointwise convergence
in probabilistic settings.

Recall that a sequence of real-valued functions $(f_n)_n$ mapping some space $\Omega$
to $\mathbb{R}$ _converges pointwise_ to $f: \Omega \to \mathbb{R}$, if for each
$\omega \in \Omega$, $f_n(\omega) \to f(\omega)$. There is no uniformity condition.

In the next definition, we assume that random
variables are real-valued. The definition
is easily extended to multivariate settings.

::: {#def-asc name="Almost sure Convergences"}

Let $(\Omega, \mathcal{F}, P)$ be a probability space, a sequence $(X_n)_n$ of random variables converges _almost surely_ (a.s.) towards a random variable $X$ if
the event
...
## Convergence in $L_p$  {#sec-Lpconvergence}

In this section, we consider random variables that satisfy  integrability assumptions.
The scope of $L_p$ convergences is narrower than the scope of $L_p$ convergences.

We already introduced $L_p$ convergences in Lesson @sec-moments. We recall it for the sake of readibility.
...
Almost sure convergence is not tied to integrability. We cannot ask whether
almost sure convergence implies $L_p$ convergence. But we can ask whether
$L_p$ convergence implies almost sure convergence. The next statement
is  a by-product  of the proof  of the completeness of $L_p$ spaces, see
...
A counter-example given in [Section @sec-lpspaces]) shows that convergence in $L_p$
does not imply almost-sure convergence.


## Convergence in probability {#sec-probaconvergence}

If we denote by $L_0=L_0(\Omega, \mathcal{F}, P)$ the set of real-valued
random variables, the notion of convergence in probability is relevant
to all sequences in  $L_0$ like almost sure convergence. And like
convergence in $L_p, p\geq 1$, convergence in probability can be metrized.
...
::: {#prp-critConvProba name="A criterion for convergence in probability"}
...
the limit of the right-hand side is not larger than $\epsilon$. As we can take
$\epsilon$ arbitrarily small, this entails that the limit of the
...
\end{array}$$
Hence $\lim_n \mathbb{E} \Big[ 1 \wedge |X_n -X|\Big] = 0$ entails
...
Assume $X_n \to X$ a.s., that is $|X_n -X| \to 0$. Then by dominated convergence,
...
Note that we have to check that $\mathrm{d}_{\mathrm{KF}}$ is indeed a distance.
This is the content of Proposition \@ref(prp:kyfanprop) below.
...
Let $a > \mathrm{d}_{\mathrm{KF}}(X, Y)$ the event $A_a = \Big\{ |X-Y| > a \Big\}$ has probability smaller than $\epsilon$. And if $\epsilon < a < b$, $A_b \subseteq  A_a$. By monotone converence, $P\Big(\cap_n A_{\epsilon + 1/n}\Big)=  \lim_{n} \uparrow P\Big(A_{\epsilon + 1/n}\Big) = \epsilon$.
...
### Convergence in probability, multivariate setting

A sequence $(X_n)_{n \in \mathbb{N}}$  of $\mathbb{R}^k$-valued random variables  living on the same probability space $(\Omega, \mathcal{F}, P)$  converges in  probability (in $\mathbb{P}$-probability) towards a
$\mathbb{R}^k$-valued  random variable $X$ iff
for every $\epsilon >0$
...
## Weak law of large numbers {#sec-wlln}
...
Let us turn to the general case. Without loss of generality, assume
all $X_n$ are centered. Let $\tau >0$  be a truncation threshold (which value will be tuned later). For each $i \in \mathbb{N}$, $X_i$ is decomposed into a sum:
$$X_i = X^\tau_i + Y^\tau_i$$
...
## Strong law of large numbers {#sec-slln}
...
### Strong  law of large numbers, direct part

If $X_1, \ldots, X_n, \ldots$  are independently, identically distributed, integrable $\mathbb{R}$-valued random variables over $(\Omega, \mathcal{F}, P)$ with expectation  $\mu$ then $P$-a.s.
...
Before proceeding to the proof of @thm-sllm, we point a general result about events
that depend on the limiting behavior of sequences of independent random variables: the so-called zero-one law. In order to state @thm-01law, we need a new notion: the tail $\sigma$ algebra.  
...
Assume $X_1, \ldots, X_n, \ldots$ are  random variables over some measured space. The tail $\sigma$-algebra (or the $\sigma$-algebra of tail events) is defined as:
...
The Strong Law of Large Numbers below tells us that under integrability and independence assumptions, this tail event has probability $1$.
...
Then, any event in the tail $\sigma$-algebra $\mathcal{T}$ has probability either $0$ or $1$.
...
Observe that $\cup_n \sigma\big(X_1, \ldots, X_n\big)$ is a $\pi$-system that generates the cylindrical $\sigma$-algebra (recall the definitions in @sec-dynkin around @thm-monotoneclasslemma).
...
We can now proceed to the proof of SLLN.

::: {.proof}

### Proof of SLLN (direct part)

The event $\Big\{ \omega : \lim_n \sum_{i=1}^n \frac{X_i}{n} = \mu \Big\}$ belongs
to the tail $\sigma$-algebra. By @thm-zero-one-law, to check  the Strong Law of Large Numbers, it
...
As $$P \{ X_i - X^i_i >0 \} = P\{ X_i >i \} = P\{ X_1 > i\} \, ,$$
...
only finitely many events $\{ X_i - X^i_i >0 \}$ are realized. Hence almost
surely, $T_n$ and $S_n$ differ by at most a bounded number of summands, and $\lim_n \uparrow (S_n - T_n)$
is finite.
...
$$\Big\{ \Big|T_{n(k)} - \mathbb{E}T_{n(k)} \Big| \geq n(k) > \epsilon \Big\}$$
occur. As this holds for each $\epsilon>0$, it holds simultaneously for all $\epsilon= 1/n$,
which implies that $\Big|T_{n(k)} - \mathbb{E}T_{n(k)} \Big|/n(k)$ converges almost surely to $0$.
...
If $n(k) \leq n < n(k+1)$, as $(S_n)_n$ is non-decreasing, $$\frac{n(k)}{n(k+1)}\frac{S_{n(k)}}{n(k)}\leq \frac{S_n}{n}\leq \frac{n(k+1)}{n(k)}\frac{S_{n(k+1)}}{n(k+1)}$$
with
...
[Theorem @thm-convslln]) shows that, under independence assumption, the conditions in [Theorem @thm-slln]) are tight.
Before proceeding to the proof of [Theorem @thm-convslln]), we state and prove the second Borel-Cantelli Lemma.
...
Let $A_1, A_2, \ldots, A_n$ be independent events from probability space $(\Omega, \mathcal{F}, P)$.
...
An outcome $\omega$ does not belong to infinitely many events $A_k$, iff $\omega \in \cup_{n} \cap_{k\geq n} A^c_k$.
By monotone convergence,
$$\begin{array}{rl}
...
### Strong  law of large numbers, converse part
...
In order to check that the $X_i$'s are integrable, it suffices to show that $$\sum_{n=0}^\infty P \big\{ X_1 > n \big\} = \sum_{n=0}^\infty P \big\{ X_n > n \big\} < \infty.$$
...
## Exponential inequalities {#sec-expineq}
...
values in a bounded interval $[a,b]$ and let
...
The upper bound on the variance of $Y$ has been established in [Section @sec-variance]).
...
Namely $$\psi'_Y(\lambda) = \frac{\mathbb{E}\Big[ (Y- \mathbb{E}Y) e^{\lambda (Y- \mathbb{E}Y)} \Big]}{\mathbb{E} e^{\lambda (Y- \mathbb{E}Y)}}
...
$P\{X =-1\} = P\{X =1\} = 1/2$. Then one may take $a=-b=1$ and $\operatorname{var}(X)  =1=\left(  b-a\right)^2/4$.
...
Let $v$ denote the upper bound on the variance.
...
The upper bound on variance follows from $\operatorname{var}(S) = \sum_{i=1}^n \operatorname{var}(X_i)$ and
from the first part of Hoeffding's Lemma.
...
Hoeffding's inequality provides interesting tail bounds for binomial random variables which are sums of independent
$[0,1]$-valued random variables. However in some cases, the variance  upper bound used in Hoeffding's inequality
is excessively conservative. Think for example of binomial random variable with parameters $n$ and $\mu/n$,
the variance upper-bound obtained from the boundedness assumption is $n$ while the true variance is $\mu$.
This motivates the next two exponential inequalities stated in [Theorem @thm-zbennett])  and [Theorem @thm-zbernstein]).
...
almost surely for all $i\leq n$. Let
$$S=\sum_{i=1}^n \left(  X_i-\mathbb{E} X_i\right)$$
...
## Bibliographic remarks {#sec-bibconvrv}

[@MR1932358] contains a thorough discussion of the various kinds of convergences that can be defined
for random variables. In particular,  [@MR1932358] offers a general perspective on topological issues
in probability spaces. [@MR1932358] also tackles the problem raised by random variables that take
values in (possibly infinite-dimensional) metric spaces.

Laws of large numbers and $0-1$-laws fit in the more general framework of ergodic theorems, see
[@MR1932358] or [@Dur10].
An important example of law of large numbers is the Asymptotic Equipartition Property (AEP) in
Information Theory.  Note that it holds for a much larger class of sources than the set
of memoryless sources (infinite product probability spaces). See [@cover:thomas:1991] or
[csiszar:korner:1981].

Introduction to exponential inequalities and their applications can be found in  [@massart:2003], [@BoLuMa13].
