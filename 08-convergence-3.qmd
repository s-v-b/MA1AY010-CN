# Convergence in distribution {#chapweakdistrib}
...
generating function of the Poisson distribution with mean $\lambda$: $s \mapsto \exp(\lambda(s-1))$.
...
In Lesson \@ref(convergences1), we equipped the set $L_0(\Omega, \mathcal{F}, P)$
...
## Weak convergence, vague convergence {#weakandvague}
...
```{definition,  tightconv, name="Weak convergence"}
A sequence of probability distributions $(P_n)_{n \in
\mathbb{N}}$ sur $\mathbb{R}^k$ converges _weakly_ towards probability distribution   $P$ (on $\mathbb{R}^k$) iff
...
```{definition  vagueconv, name="Vague convergence"}
A sequence of probability distributions $(P_n)_{n \in
...
```{example}
Consider the sequence of probability masses over the integers $(\delta_n)_{n \in \mathbb{N}}$. This sequence converges vaguely towards the null measure. It does not converge weakly.
```
...
## Convergence in distribution {#secConvDistribution}

```{definition,  weakcon, name="Tight convergence"}
...
$$ ($\mathcal{L}$  dnotes a probability distribution),
...
Lévy's continuity theorem, the major result from  Section \@ref(secLevyCont)
...
```{proposition weakconvquantiles}
A sequence of probability measures   defined by their quantile functions $(F^\leftarrow_n)_n$ converges weakly towards a probability measure defined by quantile function $F^\leftarrow$ iff $\lim_n F_n^\leftarrow(x) = F^\leftarrow(x)$ at every $x$ which is a  continuity point of $F^\leftarrow$.
```


```{exercise}
Prove  Proposition \@ref(prp:weakconvquantiles).
```

```{proposition asrepweakconv, name="Almost sure representation"}
...
When random variables $X_n$ are real-valued, Proposition \@ref(prp:asrepweakconv) follows easily from Proposition \@ref(prp:weakconvquantiles).
...
Then y Proposition \@ref(prp:weakconvquantiles), $\lim_n F_n^{\leftarrow}(\omega) = F^\leftarrow(\omega)$. This translates to
\[
...
## Lévy continuity theorem {#secLevyCont}



```{theorem  thmlevycont,  name="Lévy's continuity theorem"}
...
```{remark}
Theorem \@ref(thm:thmlevycont) asserts that weak convergence
...
```{lemma scheffe, name="Scheffé's Lemma'"}
...
```{proof, name="Proof of continuity theorem"}
...
## Refining the continuity theorem {#refineLevy}
...
```{theorem  thmlevycont2,  name="Lévy's continuity theorem, second form"}
...
```{definition utight, name="Uniform tightness"}
...
We shall use the following technical upper bound which is illustrated in Figure \@ref(fig:techdude):
\begin{equation}
	(\#eq:techsinc)
...
(ref:techdude) The proof of the truncation inequality takes advantage on easy bounds satisfied by the $\operatorname{sinc}$ function.
...
```{proposition, name="Truncation Lemma"}
...
```{proof, name="Proof of Truncation Lemma"}
...
```{proof, name="Proof of Uniform tightness Lemma"}
...
```{proof, name="Proof of the second form of the continuity theorem"}
...
Finally, we are able to invoke Theorem \@ref(thm:thmlevycont)  to conclude $P_{n}\rightsquigarrow  P \text{ as }{n \to \infty}$.
```
...
## Relations between convergences  {#relatconv}

The alternative characterizations of weak convergence provided by the
Portemanteau Theorem (Theorem \@ref(thm:portemanteau)) facilitate  the
...
\sqrt{n} \left(\frac{S_n}{n} - \mu \right) \rightsquigarrow \mathcal{N}\big(0, \sigma^2 \big) \, . s
\]
```

```{proof}
Let $\widehat{F}$ denote the characteristic function of the (common) distribution
of the random variables $((X_i-\mu)/\sigma)_i$. Recall fron Lesson \@ref(chapchar), that
...
## Cramer-Wold device  {#cramerwolddevice}
...
```{theorem cramerwold, name="Cramer-Wold"}
...
Theorem \@ref(thm:cramerwold) provides a short path to the Multivariate Central Limit Theorem.
...
## Weak convergence and transforms {#weakconvtransforms}
