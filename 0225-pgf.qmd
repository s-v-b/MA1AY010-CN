# Characterizations of discrete probability distributions  {#chappgf}
...
generating functions (Section \@ref(PGF)).
Later, we will survey more general tools: Laplace transforms (Section \@ref(laplace)) and characteristic functions
...
```{definition defpgf, name="Probability Generating Function"}
...
```{proposition hadamard, name="Hadamard's rule"}
...
```{theorem  pgfintegr, name="Integrability and probability generating functions"}
