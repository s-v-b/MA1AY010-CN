...
# Absolutely continuous probability measures   {#sec-acmeasures}
...
## Densities and absolute continuity {#sec-acontinuity}
<p>
...
Let   $\mu, \nu$ be
two positive measures on measurable space $(\Omega, \mathcal{F})$,
...
Prove [proposition @prp-chainrule].
...
## Univariate Gaussian distributions {#sec-gauss}
...
## Cumulative distribution functions and absolute continuity {#sec-cdf_ac}
...
This Theorem tells us that a cumulative distribution function is absolutely continuous in the sense of Definition @def-accdf iff the corresponding probability distribution is absolutely continuous with respect to Lebesgue
measure.
...
## Computing the density of an image probability distribution {#sec-imgdens}
...
elementary formula. This is the content of [Theorem @thm-geomchange]).
This extension is then used to establish an off-the-shelf formula for computing
the density of an image distribution in [Theorem @thm-imagedensityformula]).
...
is easy thanks to an non-trivial application of [Theorem @thm-geomchange]).
...
The proof of [Theorem @thm-imagedensityformula]) from [Theorem @thm-geomchange]) is a routine
...
\end{align*}
Now, we invoke [Theorem @thm-geomchange]):
\begin{align*}
\int_U \mathbb{I}_B(\phi(x)) f(x) \mathrm{d}\ell(x)
...
## Application: Gamma-Beta calculus  {#sec-gammabeta}
...
a. Random variable $U$ is distributed according to $\Gamma(p+q, \lambda)$ while 
a. $V$ is distributed according to $\operatorname{Beta}(p, q)$.
...
## Bibliographic remarks {#sec-bibac}
