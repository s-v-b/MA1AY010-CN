# Conditioning  {#fullconditioning}


## Defining conditional expectation {#motivcondexp}
...
The sub-$\sigma$-algebra need not be _atomic_ as in Lesson \@ref(conditioning4dummies).
We cannot define conditional probabilities by conditioning with respect to  atoms
generating $\mathcal{G}$. Our objective is nervertheless to define conditional expectations
with respect to  sub-$\sigma$-algebra $\mathcal{G}$, while retaining the nice properties
surveyed in Lesson \@ref(conditioning4dummies).

The general definition of conditional expectation starts from
the property described in Proposition \@ref(prp:espercond).

```{definition defCondExp, name="Conditional expectation"}
...
The next corollary is a consequence of Proposition \@ref(prp:nonnegcond).

```{corollary monotonecond}
...
\[ \forall n \in \mathbb{N},\quad\mathbb{E} \left[ X_{n + 1} \mid \mathcal{F} \right] \geq
   \mathbb{E} \left[ X_n \mid \mathcal{F} \right] . \]
```
...
Recall that when the conditioning sub-$\sigma$-algebra $\mathcal{G}$ is atomic, according to Proposition \@ref(prp:espercondpred),
...
```{theorem l2condexp, name="Conditional expectation for square integrable random variables"}
...
defined according to \@ref(def:defCondExp). Checking the first statement amounts to invoke
the right arguments from Hilbert spaces theory.

For the sake of self-reference, we recall basics if Hilbert spaces theory.

```{definition hilbert, name="Hilbert's space"}
...
projection Theorem below. Our starting point is the next observation (that follows from results in Lesson \@ref(moments)).
...
to a random variable from $L_2(\Omega, \mathcal{F}, P)$. The existence and unicity of
this closest $\mathcal{G}$-measurable random variable are warranted by the Projection Theorem.


```{theorem hilbertProjection, name="Projection Theorem"}
Let $E$ be a Hilbert space and $F$ a closed convex subset of
$F$. For every $x \in E$, there exists a unique $y \in F$, such that
\[
\|x - y\|= \inf_{z \in F} \|x - z\|.
\]
This unique closest point in $F$ is called the (orthogonal) projection of $x$ over $F$.
For any $z \in F$,
\[
...
\[
2 \left( \|x - z_n \|^2 +\|x - z_m \|^2 \right)
  = \|2 x - (z_n + z_m)\|^2 + \|z_n - z_m \|^2 .
\]
...
gives the following corollary which matches the first statement in Theorem \@ref(thm:l2condexp).
...
\]
By Theeorem \@ref(thm:hilbertProjection), $\mathbb{E}\left[(X-Y)\mathbb{I}_B\right]=0$.
...
```{definition varcond, name="Conditional variance"}
...
\[ \operatorname{Var} \left[ X \mid \mathcal{G} \right] = \mathbb{E} \left[
\left( X - \mathbb{E} [X \mid \mathcal{G}] \right)^2 \mid \mathcal{G}
   \right] . \]
...
```{proposition pythavariance}
Let $X \in \mathcal{L}_2(\Omega, \mathcal{F}, P)$
and $\mathcal{G} \subseteq \mathcal{F}$ a sub-$\sigma$-algebra. Then
\[
\operatorname{Var} [X] = \operatorname{Var} \Big[ \mathbb{E} \left[ X \mid
\mathcal{G} \right] \Big] + \mathbb{E} \Big[ \operatorname{Var} \left[ X
\mid \mathcal{G} \right] \Big] \, .
\]

```

```{proof}
Recall that  $\mathbb{E} \left[ \mathbb{E} \left[ X \mid \mathcal{G}
\right] \right] = \mathbb{E} \left[ X \right]$.
...
such that
\[
\forall B \in \mathcal{G}, \mathbb{E} \left[ \mathbb{I}_B Y \right] =
\mathbb{E} \left[ \mathbb{I}_B \mathbb{E} \left[ Y \mid \mathcal{G}
\right] \right]  .
\]
```

In words, conditional expectations according to Definition \@ref(def:defCondExp)
...
\[
\forall B \in \mathcal{G}', \mathbb{E} \left[ \mathbb{I}_B Y \right]
= \mathbb{E} \left[ \mathbb{I}_B \mathbb{E} \left[ Y \mid
...
To establish the theorem \@ref(thm:kolmoespcond), we use the _usual machinery_
of limiting arguments.

```{proposition monotech}
If $(Y_n)_n$ is a non-decreasing sequence of  non-negative
square-integrable random variables such as $Y_n \uparrow Y$ a.s.  then there exists a
$\mathcal{G}$-measurable random variable  $Z$ such that
\[
...
As $(Y_n)_n$ is non-decreasing, according to  Proposition \@ref(prp:nonnegcond)
...
We now proceed to the proof of Theorem  \@ref(thm:kolmoespcond).
...
and thus square-integrable. The random variable  $\mathbb{E}
\left[ Y_n \mid \mathcal{G} \right]$ is therefore well defined for each
$n$.
...
\[ \mathbb{E} \left[ X \right] = \mathbb{E} \left[ \mathbb{E}
   \left[ X \mid \mathcal{G} \right] \right] . \]
...
\[ X = \mathbb{E} \left[ X \mid \mathcal{G} \right]  \hspace{1em} P
   \text{-a.s.} \]
```
...
for every  $Y \in\mathcal{L}_1 (\Omega, \mathcal{G}, P)$, such that $\mathbb{E} \left[ |X
Y| \right] < \infty$
\[ \mathbb{E} \left[ XY \right] = \mathbb{E} \left[ Y \mathbb{E}
   \left[ X \mid \mathcal{G} \right] \right] . \]
...
$\mathcal{G}$-measurable then
\[ \mathbb{E} \left[ XY \mid \mathcal{G} \right] = Y \mathbb{E} \left[
   X \mid \mathcal{G} \right]  \hspace{1em} P \text{-a.s.} \]
```

```{proof}
As  $Y \mathbb{E} \left[ X \mid \mathcal{G} \right]$ is
$\mathcal{G}$-measurable, it  suffices to check that for every  $B \in
\mathcal{G}$,
\[ \mathbb{E} \left[ \mathbb{I}_B XY \right] = \mathbb{E} \left[
\mathbb{I}_B \left( Y \mathbb{E} \left[ X \mid \mathcal{G} \right]
\right) \right] . \]
But
\begin{eqnarray*}
\mathbb{E} \left[ \mathbb{I}_B XY \right] & = & \mathbb{E} \left[
...
## Conditional convergence theorems {#condconvtheorems}
...
```{theorem cmon, name="Monotone convergence"}
...
of conditional expectations:
\[
\lim_n \uparrow \mathbb{E} \left[ X_n \mid \mathcal{G} \right]
 = \mathbb{E}  \left[ X \mid \mathcal{G} \right]  \text{ a.s.}
\]
```
...
```{theorem condfatou, name="Conditional Fatou's Lemma"}
...
\text{ a.s.} \]
For every $n \geq k$, $X_n \geq Z_k$ a.s. Hence by the comparison Theorem (Corollary \@ref(cor:monotonecond)),
\[ \forall n \geq k \hspace{1em} \mathbb{E} \left[ Z_k \mid
\mathcal{G} \right] \leq \mathbb{E} \left[ X_n \mid \mathcal{G}
\right]   \text{ a.s.}
\]
as a countable union of  $P$-negligible events is
$P$-negligible. Hence for every $k$,
\[ \mathbb{E} \left[ Z_k \mid \mathcal{G} \right] \leq \liminf_n
\mathbb{E} \left[ X_n \mid \mathcal{G} \right]  \hspace{1em}
\text{a.s.} \]
This entails
\[ \lim_k \uparrow \mathbb{E} \left[ Z_k \mid \mathcal{G} \right] \leq
\liminf_n  \mathbb{E} \left[ X_n \mid \mathcal{G} \right]
\quad  \text{ a.s.} \]
...
```{theorem, name="Dominated convergence"}
...
of  $(X_n)_n$ and  $X$
\[ \mathbb{E} \left[ X_n \mid \mathcal{G} \right] \rightarrow
...
By the conditional monotone convergence Theorem (Theorem \@ref(thm:cmon)),
...
functions and on the monotonicity property of conditional expectation (Corollary \@ref(cor:monotonecond)).
...
```{proposition  condexpind}
If  $X$ is  independent from $\mathcal{G}$, then
\[ \mathbb{E} \left[ X \mid \mathcal{G} \right] = \mathbb{E} \left[ X
   \right] . \]
```
...
\right] \right] .
\end{align*}
Hence $\mathbb{E} \left[ X \right] = \mathbb{E} \left[ X \mid
\mathcal{G} \right]$.
```

Proposition \@ref(prp:condexpind) can be generalized to a more general setting.

```{proposition}
If sub-$\sigma$-algebra $\mathcal{H}$ is  independent from  $\sigma (
\mathcal{G}, \sigma (X))$ then
...
## Conditional probability distributions  {#condProbDistrib}

### Easy case: conditioning with respect to a discrete $\sigma$-algebra  {#easyregcondprob}
...
the conditional expectation of $\mathbb{I}_B$ with respect to  $X$. Indeed, for any kind of
 countable collection of events $(B_n)_n$ of $\mathcal{F}$,
...
$(B_n)_n$ is   non-decreasing tending  towards $B$, by Theorem  \@ref(thm:cmon),
...
In Section \@ref(jointdensity), we first review the easy case, where we can
define conditional probabilities that even have a density
with respect to a reference measure. In Section \@ref(regconprob) we shall see that if $\Omega$
is not too large, we can rely on  the existence of conditional probabilities.



### Joint density setting  {#jointdensity}
...
```{theorem conddensity, name="Conditional density"}
Let be $X, Y$ be the projection coordinates on $\mathbb{R}^2$. Let  $P$
be an absolutely continuous distribution on $(\mathbb{R}^2, \mathcal{B}(\mathbb{R}^2))$
with density $p (., .)$ with respect to Lebesgue's measure.
...
\begin{enumerate}[i.)]
\item For each $x$ such that  $p_X (x) > 0$, the set function $P_{\cdot \mid X=x}$
 defined by
\begin{align*}
...
\item  For every $B \in \mathcal{B} ( \mathbb{R}^2)$, the function
\[
...
\]
\item
For any  $P$-integrable function $f$ on $\mathbb{R}^2$, the
...
The proof of Theorem \@ref(thm:conddensity) consists of milking the Tonelli-Fubini Theorem.
...
```{definition, name="Regular conditional probability"}
...
\]
This countable collection of half-lines  is a $\pi$-system (See Section \@ref(dynkin)) that generates $\mathcal{B}(\mathbb{R})$.
...
The Monotone class Theorem (Section \@ref(dynkin)) tells us that $\mathcal{F \subseteq \mathcal{D}}$.
...
## Efron-Stein-Steele inequalities {#ess}
...
```{theorem zefron-stein, name="Efron-Stein-Steele's inequalities"}
Let $X_1,\ldots,X_n$
...
## Bounded differences inequalities {#mcdiarmid}
...
```{theorem, name="Bounded differences inequalities"}
...
We adopt the same notation as in Section \@ref(ess).
\begin{align*}
